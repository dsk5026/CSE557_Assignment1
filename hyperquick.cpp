...
if (rank >= midprocessor)
   ...
   leftLength=temp[0];
   MPI_Recv(bufIn, leftLength, MPI_INT, partner, 0, MPI_COMM_WORLD, &status);
   // Receive length of rightList
   myList = new int[rightLength+leftLength];
   Merge (myList, bufIn, rightList, leftLength, rightLength);
...
else
{
   ...
   // Send rightList
   int bufIn[rightLength];
   MPI_Recv(bufIn, temp[0], MPI_INT, partner, 0, MPI_COMM_WORLD, &status);
...
myList = new int[rightLength+leftLength];
Merge (myList, leftList, bufIn, leftLength, rightLength);
...
diff=tsFloat(end)-tsFloat(start);
cout<<diff <<" is the time to run parallel portion"<<endl;
}
