...
void pqsort2 (int startprocessor, int endprocessor, int* myList, int listsize)
{
   ...
   if (pdistance >= 1)
   {
      ...
      cout<< rank << " has left "<<leftLength << " and right " << rightLength<<endl;
      if (rank >= midprocessor)
      {
         ...
         buffer[0]=leftLength;
         cout<<rank << " is passing out size "<<buffer[0]<<endl;
         ...
         {
            outBuf[i]=leftList[i];
            cout<<rank << " is preparing to send out "<<outBuf[i]<<endl;
         }
         ...
         cout<<rank<<" has incoming of size "<<leftLength<<endl;
         MPI_Recv(inList, leftLength, MPI_INT, partner, 0, MPI_COMM_WORLD, &status);
         for (int i = 0; i < leftLength; i++)
         {
            cout<<"i is "<<i<<endl;
            cout<<rank << " got " <<inList[i]<<endl;
         }
         myList = new int[leftLength+rightLength];
      ...
      else
      {
         ...
         cout<<rank<<" has incoming of size "<<rightLength<<endl;
         for (int i = 0; i < leftLength; i++)
         {
            cout<<"i is "<<i<<endl;
            cout<<rank << " got " <<inList[i]<<'\t'<<endl;
